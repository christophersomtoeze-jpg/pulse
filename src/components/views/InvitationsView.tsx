...
import { Ban, Mail, UserPlus } from 'lucide-react';
import {
  inviteToWorkspace,
...
const ROLE_OPTIONS: { value: WorkspaceRole; label: string; hint: string }[] = [
  ...
  { value: 'member', label: 'Member', hint: 'Create decisions & vote' },
  { value: 'guest', label: 'Guest', hint: 'Limited participation' },
...
export function InvitationsView({ workspaceId }: { workspaceId: string }) {
  ...
  const submit = async (e: FormEvent) => {
    ...
    try {
      ...
      setNotice(
        result.added
          ? 'Existing PULSE user added to the workspace.'
          : 'Invitation saved. They can join when they sign up with this email.',
      );
      load();
    } catch (err) {
  ...
  const revoke = async (invite: WorkspaceInvite) => {
    ...
    setError('');
    try {
      await revokeWorkspaceInvite(invite.id, workspaceId);
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 pb-28 pt-5">
      <p className="flex items-center gap-1.5 text-xs uppercase tracking-[.2em] text-pulse-300">
        <UserPlus className="h-3.5 w-3.5" /> Workspace
      </p>
      <h1 className="mt-1 font-display text-2xl font-semibold">Invitations</h1>
      <p className="mt-1 text-sm text-ink-400">
        Invite people by email and assign a role. Viewers are read-only; admins can manage the workspace.
      </p>
      ...
      <form onSubmit={submit} className="glass mt-5 rounded-2xl p-4">
        <div className="flex flex-col gap-2 sm:flex-row">
          ...
          />
          <select value={role} onChange={(e) => setRole(e.target.value as WorkspaceRole)} className="field sm:w-36">
            {ROLE_OPTIONS.map((r) => (
          ...
          <button disabled={busy} className="primary-btn justify-center disabled:opacity-40">
            {busy ? 'Sending…' : 'Invite'}
          </button>
        </div>
        <p className="mt-2 text-[11px] text-ink-500">{ROLE_OPTIONS.find((r) => r.value === role)?.hint}</p>
      ...
      <div className="mt-5 space-y-2">
        {invites.map((i) => (
          <div key={i.id} className="glass flex items-center gap-3 rounded-2xl p-3.5">
            <div className="grid h-8 w-8 shrink-0 place-items-center rounded-xl bg-alert-500/10">
              <Mail className="h-4 w-4 text-alert-300" />
            </div>
            <div className="min-w-0 flex-1">
              <p className="truncate text-sm font-medium">{i.email}</p>
              <p className="text-[10px] uppercase tracking-wide text-ink-500">
                {i.role} · {i.status} · {new Date(i.createdAt).toLocaleDateString()}
              </p>
            </div>
            {i.status === 'pending' && (
              <button type="button" disabled={busy} onClick={() => void revoke(i)} className="secondary-btn text-xs">
